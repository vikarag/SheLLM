...
"""Interactive chat with DeepSeek Reasoner (V3.2 thinking mode) - maintains conversation history."""
...
# deepseek-reasoner does not support function calling; only deepseek-chat does
...
def handle_tool_calls(response_message, messages):
    """Process tool calls from the model, execute search, and get final response."""
...
def handle_batch(response):
    ...
    if getattr(message.message, "tool_calls", None):
        return None, message.message

    reasoning = getattr(message.message, "reasoning_content", None)
    if reasoning:
        print(f"\n[Thinking]\n{reasoning}")

    answer = message.message.content or ""
    print(f"\nAssistant: {answer}\n")
    return answer, None
...
def main():
    messages = []
    tools_note = " | Auto web search enabled" if SUPPORTS_TOOLS else ""
    print(f"DeepSeek Chat (model: {MODEL}, stream: {STREAM}, temp: {TEMPERATURE or 'default'}{tools_note})")
