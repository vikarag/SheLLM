...
API_KEY = os.environ.get("INCEPTION_API_KEY", "YOUR_API_KEY_HERE")

client = OpenAI(api_key=API_KEY, base_url="https://api.inceptionlabs.ai/v1")

# ── Settings ────────────────────────────────────────────────────────
MODEL = "mercury-2"          # Model ID
...
def main():
    messages = []
    print(f"Mercury-2 Chat (model: {MODEL}, stream: {STREAM}, temp: {TEMPERATURE or 'default'})")
